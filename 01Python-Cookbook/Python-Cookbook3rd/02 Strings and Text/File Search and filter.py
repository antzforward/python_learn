...
# print(f"os walk：耗时：{time.time() - start_time}秒") #os walk：耗时：11.511752605438232秒
...
def os_scandir_all_root_while(top ):
    stack = [(top,False)]  # 使用列表作为栈来存储待处理的目录
    while stack:  # 只要栈不为空，就继续处理
        current_dir,matched = stack.pop()  # 弹出当前目录
        try:
        ...
        for item in items:
            if item.is_file():
                ...
                stack.append((item.path.replace('\\','/'),childMatched))  # 将子目录压入栈中，以便后续处理
...
def Path_iterdir_all_root_while(top):
    stack = [(Path(top), False)]  # 使用列表作为栈来存储待处理的目录
    while stack:  # 只要栈不为空，就继续处理
        current_dir, matched = stack.pop()  # 弹出当前目录
        ...
        try:
            for item in items:
                if item.is_file():
                    ...
                    childMatched = matched or headsRe.match(item.name)
                    stack.append((item, childMatched))  # 将子目录压入栈中，以便后续处理
        except (PermissionError, FileNotFoundError, NotADirectoryError) as e:
            continue
...
# print(f"path  iterdir by BFS：耗时：{time.time() - start_time}秒") #path  iterdir by BFS：耗时：28.875741958618164秒
...
# print(f"pathlib walk：耗时：{time.time() - start_time}秒") #pathlib walk：耗时：15.812865018844604秒
...
print(f"fd walk：耗时：{time.time() - start_time}秒") #fd walk：耗时：1.0895521640777588秒
print( len(os_walk_res6) ) #6768
